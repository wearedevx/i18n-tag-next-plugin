...
function traverseDir(rootPath) {
  ...
  var pathTree = fs.readdirSync(dirPath).filter(function (file) {
    ...
    return prependSlash(subPath.replace(/\\/g, '/').replace('index.js', '').replace(/\.m?jsx?/g, ''));
  });
...
function localizedPathMaps() {
  var paths = traverseDir(path.join(__dirname, '..', '..', 'pages', '[lang]'));
            ...
            return regeneratorRuntime.wrap(function _callee$(_context) {
              while (1) {
                switch (_context.prev = _context.next) {
                  case 0:
                    return _context.abrupt("return", _objectSpread({}, defaultPathMap, {}, localizedPathMaps()));
